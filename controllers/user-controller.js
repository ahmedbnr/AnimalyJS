...
exports.login = async (req, res) => {
  ...
  if (user && (await bcypt.compare(password, user.password))) {
    ...
    if (user.isVerified) {
      ...
      res.status(201).send({ user, message: "Email not verified" });
    }
  } else {
...
exports.editProfile = async (req, res) => {
  const { firstName, lastName, cin,oldEmail, newEmail, address, phoneNumber } = req.body;
  ...
  else
  {
    if(oldEmail != newEmail)
    {
      ...
      const token = jwt.sign({ email: newEmail }, config.token_secret, {
        expiresIn: "36000000",
      });
...
exports.editProfilePicture = async (req, res, next) => {
  ...
  if(user != null)
  {
    ...
    res.send({message:"success", user: user, profilePicture: req.file.filename });
